...
public class AbServiceImple implements AbService {
	...
	public ResponseSchema<?> insertAb(Ab input) {	
		...
		//Jika ab_id NILAINYA TIDAK STRING KOSONG ATAU HANYA BERISI SPASI , maka gak bisa create
		if(!(input.getAbId().length() == 0 || input.getAbId().contains(" "))) {
			...
			//KONDISI APABILA PANJANG INPUT PARAM  TIDAK MELEBIHI BATAS MAXIMAL VALUE DARI MASING-MASING KOLOM
			if(!(input.getAbName().length() >=41 || input.getAbId().length() >= 6 || input.getIsNotified().length() >=2 || input.getNotifTransaksi().length() >= 43 || input.getCorporateId().length() >= 11 || input.getCompCode().length() >= 11 || input.getAbInitial().length() >= 21)) {
				...
				if(abRepository.cekAbId(input.getAbId().toUpperCase()).size() < 1  ) {
					...
					//NILAI IS_NOTIFIED HANYA UNTUK "Y", "N"
					if((input.getIsNotified().equals("y") || input.getIsNotified().equals("n") || input.getIsNotified().equals("Y") || input.getIsNotified().equals("N") )){
						...
						System.out.println("["+valueLog + "]" +" - "+dateLog() +" - Sebelum cetak fungsiNotifTransaksi, Notif_Transaksi = " + input.getNotifTransaksi().toString());
						dataAb.setNotifTransaksi(fungsiNotifTransaksi(input.getNotifTransaksi().toUpperCase()));
						System.out.println("["+valueLog + "]" +" - "+dateLog() +" - Set Notif Transaksi dari createNotifTransaksi = "+ fungsiNotifTransaksi(input.getNotifTransaksi()));
						dataAb = abRepository.save(dataAb);
						System.out.println("["+valueLog + "]" +" - "+dateLog() + " - Save ke db selesai, isi dari dataAb = "+dataAb.toString());
					}
					//Else Untuk VALUE IS_NOTIFIED SELAIN (Y, N, y, n)
					else {
...
//				
			...
			//JIKA DATA YANG DIINPUT DI INPUT PARAM(BODY) ADA YANG MELEBIHI BATAS MAX DARI SETIAP KOLOM, MAKA BISA CREATE AB
			else {
	...
	public Ab findAbById(String id) {
		return abRepository.findById(id).get();
	}
	...
	//Update  pakai inputan param nya MAP
		public ResponseSchema<?> updateAb (Map<String, Object> map) throws ParseException {
			...
			Ab objAb = findAbById(map.get("ab_id").toString().toUpperCase());
			...
			//ELSE -> BISA UPDATE DATA KARNA PANJANG INPUT PARAM NYA TIDAK MELEBIHI MAX VALUE DARI TIAP KOLOM.
				...
				if(!(map.get("ab_id").toString().toUpperCase().equals(objAb.getAbId()) ) ) {
			...
			if(!(map.get("ab_name") == null) && penanda[1].equals("Terupdate")) {
				System.out.println("["+valueLog + "]" + " - "+dateLog() + " - ab_name_old = " + uia.getAbName() );
				map2.put("ab_name_old", uia.getAbName());
//				map2.put("ab_name_new", map.get("ab_name").toString());
				System.out.println("["+valueLog + "]" + " - "+dateLog() + " - ab_name_new = " + objAb.getAbName() );
			...
			if(!(map.get("is_notified") == null) && penanda[2].equals("Terupdate")) {
				System.out.println("["+valueLog + "]" + " - "+dateLog() + " - is_notified_old = " + uia.getIsNotified() );
				map2.put("is_notified_old", uia.getIsNotified());
				System.out.println("["+valueLog + "]" + " - "+dateLog() + " - is_notified_new = " + objAb.getIsNotified() );
			...
			if(!(map.get("notif_transaksi") == null) && penanda[3].equals("Terupdate")) {
				System.out.println("["+valueLog + "]" + " - "+dateLog() + " - notif_transaksi_old = " + uia.getNotifTransaksi() );
				map2.put("notif_transaksi_old", uia.getNotifTransaksi());
				System.out.println("["+valueLog + "]" + " - "+dateLog() + " - notif_transaksi_new = " + objAb.getNotifTransaksi() );
			...
			if(!(map.get("corporate_id") == null) && penanda[4].equals("Terupdate")) {
				System.out.println("["+valueLog + "]" + " - "+dateLog() + " - corporate_id_old = " + uia.getCorporateId() );
				map2.put("corporate_id_old", uia.getCorporateId());
				System.out.println("["+valueLog + "]" + " - "+dateLog() + " - corporate_id_old = " + objAb.getCorporateId() );
			...
			if(!(map.get("comp_code") == null) && penanda[5].equals("Terupdate")) {
				System.out.println("["+valueLog + "]" + " - "+dateLog() + " - comp_code_old = " + uia.getCompCode() );
				map2.put("comp_code_old", uia.getCompCode());
				System.out.println("["+valueLog + "]" + " - "+dateLog() + " - comp_code_new = " + objAb.getCompCode() );
			...
			if(!(map.get("ab_initial") == null) && penanda[6].equals("Terupdate")) {
				System.out.println("["+valueLog + "]" + " - "+dateLog() + " - ab_initial_old = " + uia.getAbInitial() );
				map2.put("ab_initial_old", uia.getAbInitial());
				System.out.println("["+valueLog + "]" + " - "+dateLog() + " - ab_initial_new = " + objAb.getAbInitial() );
				map2.put("ab_initial_new", objAb.getAbInitial().toUpperCase());
			}
			
			//KONDISI UNTUK SEMUA PENANDA GAGAL UDPADTE
		
			if("Gagal".equals(penanda[1]) && "Gagal".equals(penanda[2]) && "Gagal".equals(penanda[3]) && "Gagal".equals(penanda[4]) && "Gagal".equals(penanda[5]) && "Gagal".equals(penanda[6]) ) {
	...
	public Ab deleteAb(String id) {
		...
		Ab objAb = findAbById(id);
		System.out.println("["+valueLog + "]" + " - "+dateLog()  +" - Data berhasil ditemukan : " + objAb.toString());
